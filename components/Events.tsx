...
export default function Events() {
  const [mode, setMode] = useState<EventCategory>("tech");
  const [selected, setSelected] = useState<FestEvent | null>(null);

  const filtered = useMemo(() => events.filter((e) => e.category === mode), [mode]);

  return (
    <section id="events" className="py-28 md:py-40 relative overflow-hidden">
      <div
        className="absolute inset-0 pointer-events-none transition-colors duration-700"
        style={{
          background:
            mode === "tech"
              ? "radial-gradient(ellipse 50% 40% at 85% 5%, rgba(0,243,255,0.08), transparent 70%)"
              : "radial-gradient(ellipse 50% 40% at 85% 5%, rgba(255,0,60,0.08), transparent 70%)",
        }}
      />
      <div className="container-content relative">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-14">
          ...
          >
            ...
            <h2 className="text-[11vw] sm:text-5xl md:text-6xl leading-[0.98]">
              CHOOSE
              <br />
              YOUR
              <br />
